...
module sg_segment_walker #(
    ...
    wire page_has_min_burst =
        (current_addr[11:0] <= (12'h1000 - MIN_BURST_BYTES));
                ...
                if (burst_contract_ok) begin
                    if (seg_bytes_left > burst_bytes) begin
                        ...
                        seg_valid <= (seg_bytes_left - burst_bytes >= MIN_BURST_BYTES) &&
                                     ((current_addr[11:0] + burst_bytes) <=
                                      (12'h1000 - MIN_BURST_BYTES));
                    ...
                    // Fail closed instead of silently consuming the next SGL entry.
                    seg_valid <= 1'b0;
                end
            end else begin
                count <= sgl_wr_en ? (count + 1'b1) : count;
